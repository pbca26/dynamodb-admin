...
const DEFAULT_EXPORT_FORMAT = process.env.DEFAULT_EXPORT_FORMAT || 'json'
...
exports.createServer = (dynamodb, docClient) => {
  ...
  app.use(function (req, res, next) {
    const { theme = DEFAULT_THEME, exportFormat = DEFAULT_EXPORT_FORMAT } = req.cookies
    res.locals = {
      theme,
      exportFormat,
    }
  ...
  app.get('/tables/:TableName/export', asyncMiddleware((req, res) => {
    const { exportFormat = DEFAULT_EXPORT_FORMAT } = req.cookies

    return exportTable(req.params.TableName, dynamodb, exportFormat)
      .then(data => {
